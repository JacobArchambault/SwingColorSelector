...
public class ColorSelector extends JFrame {
	...
	public static void main(
		...
		JFrame.setDefaultLookAndFeelDecorated(
				true);
		ColorSelector frameInfo = new ColorSelector();
		frameInfo.setSize(
				450,
				250);
	...
	public ColorSelector() {
		...
		colorLabel = new JLabel(
				"Welcome to Your Favorite Colors!");
	...
	public void createButtons() {
		...
		colorButtons[0] = new JButton(
				"blue");
		colorButtons[0].setBackground(
				Color.blue);
		colorButtons[1] = new JButton(
				"black");
		colorButtons[1].setBackground(
				Color.black);
		...
		colorButtons[3] = new JButton(
				"gray");
		colorButtons[3].setBackground(
				Color.gray);
		colorButtons[4] = new JButton(
				"red");
		colorButtons[4].setBackground(
				Color.red);
		colorButtons[5] = new JButton(
				"magenta");
		colorButtons[5].setBackground(
				Color.magenta);
		for (int i = 0; i < colorButtons.length; i++) {
			colorButtons[i].setForeground(
					Color.white);
		}
		for (int i = 0; i < colorButtons.length; i++) {
			centerPanel.add(
						...
						public void actionPerformed(
							...
							for (int i = 0; i < colorButtons.length & !foundKey; i++) {
								if (e.getSource() == colorButtons[i]) {
									foundKey = true;
									colorField.setBackground(
											colorButtons[i].getBackground());
									colorField.setText(
											colorButtons[i].getText() + " is such a lovely color");
									colorField.setForeground(
											colorButtons[i].getForeground());
								}
							} // closes for loop
	...
	void createMenu() {
		...
		aboutMenu = new JMenu(
				"About");
		changeColors = new JMenu(
				"Change Colors");
		displayMenu = new JMenuItem(
				"Display");
		clearMenu = new JMenuItem(
				"Clear");
		closeMenu = new JMenuItem(
				"Exit");
		aboutColorMenu = new JMenuItem(
				"About Color Buttons");
		changeTextFieldColor = new JMenuItem(
				"Change Field Color");
		...
		aboutMenu.setMnemonic(
				'A');
		changeColors.setMnemonic(
				'H');
		displayMenu.setMnemonic(
				'D');
		clearMenu.setMnemonic(
				'C');
		closeMenu.setMnemonic(
				'X');
		aboutColorMenu.setMnemonic(
				'B');
		...
		displayMenu.addActionListener(
				new ActionListener() {
					public void actionPerformed(
							ActionEvent e) {
						colorField.setText(
								"Press the key for your favorite color");
						colorField.setBackground(
								Color.white);
						colorField.setForeground(
								Color.black);
		...
		aboutColorMenu.addActionListener(
				new ActionListener() {
					public void actionPerformed(
							ActionEvent e) {
		...
		changeTextFieldColor.addActionListener(
				new ActionListener() {
					public void actionPerformed(
							ActionEvent e) {
